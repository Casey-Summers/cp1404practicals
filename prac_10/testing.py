...
def repeat_string(s, n):
    ...
    return " ".join([s] * n)  # complete task 1 using " ".join
...
def is_long_word(word, length=5):
    ...
    True
    """
    return len(word) >= length  # complete task 4 by setting >= length
...
def format_phrase_as_sentence(phrase):
    """Formats a phrase into a sentence, starting with a capital and ending with a single full stop.
    >>> format_phrase_as_sentence('hello')
    'Hello.'
    >>> format_phrase_as_sentence('It is an ex parrot.')
    'It is an ex parrot.'
    >>> format_phrase_as_sentence('i am become death')
    'I am become death.'
    """
